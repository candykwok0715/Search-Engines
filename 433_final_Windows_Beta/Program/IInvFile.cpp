			...
			while (w != NULL) {	// Loop through each hash node (hnode) in the linked list
				...
				while (k != NULL) {
					if (k->docid <= MaxDocid) {
						...
						if (k->freq > File[k->docid].tf_max)
							File[k->docid].tf_max = k->freq;
					} else
...
void IInvFile::LoadDocRec(char * f) {
	...
	if (fgets(line, 10000, fp) != NULL) {
		...
		while (fgets(line, 10000, fp) != NULL) {
			sscanf(line, "%s %e %d", str, &doclen, &tf_max);
			Files[i].TRECID = strdup(str);
			Files[i].len = doclen;
			Files[i].tf_max = tf_max;
			if (i > MaxDocid)
	...
	if ((MaxDocid > 0) && (Files != NULL)) {
		...
		for (int i = 0; i <= MaxDocid; i++) {
			if (Files[i].TRECID == NULL)
				fprintf(fp, "%d %e %d\r\n", i, Files[i].len, Files[i].tf_max);
			else
				fprintf(fp, "%s %e %d\r\n", Files[i].TRECID, Files[i].len, Files[i].tf_max);
		}
		fclose(fp);
	}
}
...
char * IInvFile::GotoNextWord(char * s) {
	...
	if ((s == NULL) || (*s == '\0') || (*s == '\r') || (*s == '\n'))
		return NULL;
	if ((*q > 32 && *q < 65) || (*q > 90 && *q < 97) || (*q > 122 && *q < 127)) {
		*q = ' ';
	}
	if ((*q == ' ')) {
		while ((*q == ' ') || (*q == '\t') || (*q == '\r') || (*q == '\n')) {
...
void IInvFile::CombineResult(RetRec * r, post * kk, float idf) {
	...
	while (k != NULL) {
		...
		r[docid].sim += (idf * ((float) k->freq / Files[docid].tf_max));			// add the partial dot product score
		k = k->next;				// next posting
	}
}
...
// Print top N results
void IInvFile::PrintTop(RetRec * r, int top, int id, FILE * fp) {
	...
	while (i < top) {
		...
		fprintf(fp, "%d Q0 %s %d %e GroupE\n", id, Files[r[i].docid].TRECID, i + 1, r[i].sim);
		//printf("%d Q0 %s %d %e GroupE\n", id, Files[r[i].docid].TRECID, i + 1, r[i].sim);
		i++;
	}
}

// Perform retrieval
void IInvFile::Search(char * q, int id, FILE * fp) {
	char * s = q;
	char * w;
	bool next = true;
	hnode * h;
	...
	PrintTop(result, 1000, id, fp);				// Print top 10 retrieved results
}
...
void IInvFile::ReadTRECID(char * f) {
	char line[10000], str[1000], TrecId[1000];
	int docid = 0, len = 0;
	FILE *fp = fopen(f, "r");
	if (fp == NULL) {
